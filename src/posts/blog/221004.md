...
title: "HTTP 헤더 분석"
category: "네트워크"
date: "2022-10-04 12:00:00 +09:00"
desc: "Hyper Text Transfer Protocol, Header"
thumbnail: "./images/thumbnail/network.png"
alt: "6"
---
...
HTTP는 Application layer 프로토콜이며 TCP / IP 위에서 작동한다. (포스트의 주제가 HTTP 헤더 분석인지라 계층과 관련된 자세한 내용은 [OSI_모형](https://ko.wikipedia.org/wiki/OSI_%EB%AA%A8%ED%98%95) 참고.) HTTP는 80번 포트를 사용하며, 요청(Request)과 응답(Response)으로 구성되어 데이터를 주고받는다. 클라이언트가 80번 포트로 요청을 보내면, 서버에서 응답을 하는 방식이다.
...
영문에서 알 수 있듯이 제 3자가 정보를 볼 수 없도록 암호화가 첨가되었다. 80번 포트가 아닌 443번 포트를 사용하며, 보안 소켓 계층(SECURE SOCKETS LAYER)을 사용한다. 이 SSL 인증서는 사용자가 사이트에서 제공하는 정보를 암호화하는데, 제 3자가 개인정보에 접근했을 경우에도 데이터가 이미 암호화된 상태이기 때문에 확인이 불가능하다. 전송 계층 보안(Transport Layer Security) 또한 사용되는데, 클라이언트와 서버 간의 커뮤니케이션을 암호화해준다.

## 네트워크 헤더

브라우저 개발자 도구의 네트워크 탭에서 헤더를 확인할 수 있는데, 요청과 응답의 부가적인 정보가 명시되어 있다. 다음은 네이버의 헤더 내용이다.
