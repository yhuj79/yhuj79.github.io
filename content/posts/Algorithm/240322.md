...
진행 흐름을 나타내 본다면 위 그림과 같을 것이다. 이제 C언어로 버블 정렬을 수행하는 코드를 작성하여 `6, 5, 3, 1, 8, 7, 2, 4`를 정렬해 보자.
...
주어진 배열을 버블 정렬 알고리즘을 사용하여 정렬한다. `bubbleSort()` 함수는 배열을 받아서 버블 정렬을 수행하고, `printArray()` 함수는 배열의 요소를 출력한다. `main()` 함수에서는 주어진 배열을 초기화하고 정렬 후 결과를 출력한다.
...
10000, 20000, 40000 ... 으로 값을 2배씩 증가시켜 보았다. 버블 정렬의 시간 복잡도는 O(N²)으로, 데이터의 양이 증가함에 따라 실행 시간이 제곱으로 증가한다. 하지만 정확한 제곱을 뜻하는 것은 아니고, 데이터의 크기가 2배로 증가하면 걸리는 시간은 일반적으로 4배가 아닌 4배보다 더 많이 증가한다고 보면 된다. 위의 실행 결과처럼.
...
function measureTimeTaken(callback) {
  ...
  callback()
  var endTime = performance.now()
  return (endTime - startTime) / 1000 // milliseconds to seconds
}
...
삽입 정렬 또한 O(N²)의 시간복잡도를 갖는다. 역시 O(N²)들 중 빠른 편에 속하는 삽입 정렬 답게 포스트에서 다룬 알고리즘들 중 가장 빠른 결과값이 나왔다. 최선의 경우 O(N)이라는 엄청나게 빠른 효율성을 가지고 있기 때문이다. 하지만 이 정렬의 특성 상 입력 데이터가 역순으로 정렬되어 있다면 결국 최악의 성능이 나온다.
